...
如果同事扫码后在飞书侧直接看到“没有该应用权限”之类提示，而不是回到网页显示登录失败，通常不是代码问题，而是飞书应用可用范围没有包含该成员。需要到飞书开放平台或飞书管理后台，把应用改成全员可用，或把对应成员/部门加入可用范围，然后按要求重新发布应用。
...
管理员现在也可以直接在系统设置页点击“发送测试提醒”：

- 会调用 `send-reminders`
- 使用当前选择的本地时间作为测试时间
- 只向命中的到期/逾期任务负责人发送真实飞书单人消息
- 适合在启用 Cron 之前先验证飞书权限、成员 `open_id` 和消息链路
...
- 成员邮箱
- 成员手机号
- 成员页里手工填写到“飞书 ID / 邮箱”的 `open_id` 或 `user_id`
